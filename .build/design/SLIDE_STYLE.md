...
- **Aspect ratio 16:9** for every slide. L1 and L2 are the only lessons
  with slides: both are video-only, with no notebook beside the slide,
  so the wide format lays their flows out left-to-right. Generous
  margins; never more than one diagram or chart per slide.
- **L3 through L5 and the appendix ship no slides.** They are presented
  live from the notebooks, and the executed outputs carry the visuals.
  Notebook figures are 8 units wide to match the 8:9 recording frame,
  which is a figure size rather than a slide format.
